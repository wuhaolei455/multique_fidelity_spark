import copy
import numpy as np
from typing import Optional, Dict, Any
from ConfigSpace import Configuration

from .base import Evaluator

_DEFAULT_EXTRA_INFO = {'qt_time': {}, 'et_time': {}}


class MockExecutor(Evaluator):
    def __init__(self, seed: Optional[int] = None):
        self.rng = np.random.default_rng(seed)
        self.sql_list = ['q10', 'q11', 'q12', 'q13', 'q14a', 'q14b', 'q15', 'q16', 'q17', 'q18', 'q19', 'q1', 'q20', 'q21', 'q22', 'q23a', 'q23b', 'q24a', 'q24b', 'q25', 'q26', 'q27', 'q28', 'q29', 'q2', 'q30', 'q31', 'q32', 'q33', 'q34', 'q35', 'q36', 'q37', 'q38', 'q39a', 'q39b', 'q3', 'q40', 'q41', 'q42', 'q43', 'q44', 'q45', 'q46', 'q47', 'q48', 'q49', 'q4', 'q50', 'q51', 'q52', 'q53', 'q54', 'q55', 'q56', 'q57', 'q58', 'q59', 'q5', 'q60', 'q61', 'q62', 'q63', 'q64', 'q65', 'q66', 'q67', 'q68', 'q69', 'q6', 'q70', 'q71', 'q72', 'q73', 'q74', 'q75', 'q76', 'q77', 'q78', 'q79', 'q7', 'q80', 'q81', 'q82', 'q83', 'q84', 'q85', 'q86', 'q87', 'q88', 'q89', 'q8', 'q90', 'q91', 'q92', 'q93', 'q94', 'q95', 'q96', 'q97', 'q98', 'q99', 'q9']

    def __call__(self, config: Configuration, resource_ratio: float, plan: Optional[Dict[str, Any]] = None, **kwargs) -> Dict[str, Any]:
        extra_info = copy.deepcopy(_DEFAULT_EXTRA_INFO)

        plan_sqls = []
        timeouts = {}
        if isinstance(plan, dict):
            plan_sqls = list(plan.get('sqls') or [])
            timeouts = dict(plan.get('timeout') or {})

        if not plan_sqls:
            plan_sqls = list(self.sql_list)

        extra_info['plan_sqls'] = list(plan_sqls)
        extra_info['plan_timeout'] = timeouts

        for sql_name in self.sql_list:
            if sql_name in plan_sqls:
                qt = float(self.rng.random())
                et = float(self.rng.random())
            else:
                qt = float('inf')
                et = float('inf')
            extra_info['qt_time'][sql_name] = qt
            extra_info['et_time'][sql_name] = et

        finite_qt_values = [value for value in extra_info['qt_time'].values() if np.isfinite(value)]
        if finite_qt_values:
            result_objective = float(sum(finite_qt_values))
            timeout = False
        else:
            result_objective = float('inf')
            timeout = True

        return {
            'result': {'objective': result_objective},
            'timeout': timeout,
            'traceback': None,
            'extra_info': extra_info,
            'elapsed_time': float(self.rng.random()),
        }